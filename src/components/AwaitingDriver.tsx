...
const AwaitingDriver: React.FC = () => {
    const { language, setAppState, rideDetails } = useAppContext();
        ...
        <div className="w-full max-w-lg mx-auto text-center animate-fade-in flex flex-col items-center">
            <div className="relative w-40 h-40 mb-6 flex items-center justify-center">
                ...
                <div 
                    ...
                    style={{ animation: 'radar-pulse 2s infinite ease-out' }}
                />
                <div 
                    className="absolute w-full h-full rounded-full border-2 border-blue-500" 
                    style={{ animation: 'radar-pulse 2s infinite ease-out', animationDelay: '0.5s' }}
                />
                <div 
                    className="absolute w-full h-full rounded-full border-2 border-blue-500" 
                    style={{ animation: 'radar-pulse 2s infinite ease-out', animationDelay: '1s' }}
                />
            ...
            <h2 className="text-2xl font-bold text-white mb-2">
                {t('awaiting_driver_title', language as 'en' | 'es')}
            </h2>
            <p className="text-gray-400 mb-6">
                {t('awaiting_driver_subtitle', language as 'en' | 'es')}
            </p>

            <div className="bg-gray-800/50 rounded-lg p-3 text-center w-full mb-6">
                <p className="text-sm text-gray-400">
                    {t('your_offer_was', language as 'en' | 'es')}
                </p>
                <p className="text-2xl font-bold text-emerald-400">
                    Bs. {rideDetails.suggestedFare.toFixed(2)}
                </p>
            </div>
            ...
            >
                {t('cancel_request_button', language as 'en' | 'es')}
            </button>
        </div>
    );
};
